...
foreach $stp (@stptr){
	if ($stp eq "indref"){
		...
		use concat;
		if (not defined ($inputfolder && $outputfolder)){print "\nThis script will concatenate fastq files of raw or trimmed reads of several samples in parallel. It requires fastq files of all samples stored in the same folder if they are already trimmed, or in a folder with each sample in its on folder when they are raw fastq files.\n\nUsage:\nMELFUwgrs.pl -stp concat\n\t-i <path to the folder with the input fastq files, or with the folders for each sample>\n\t-o <path to the output folder>\n Optional:\n\t-snc <number of runs in parallel, default 10>\n\t-t <method used for trimming. Trimmomatic TR, AdapterRemoval AR or None NO if are raw sequences, default AR>\n\t-exf <this will tell the script how to extract the name of each sample, and should include all extra information at the end of the file names that is not related to the sample name, default P1_L001_ NOTE for raw data paired-end, this should include the extension for both reads separated by a comma, example _1.fq.gz,_2.fq.gz, we will use the folder name as the name of the concatenate output.>\n\nFor example:\nMELFUwgrs.pl -stp concat -i /home/Yumafan/demultiplex/trimmed/ -o /home/Yumafan/concatenated -snc 10 -t AR -exf P1_L001_\n\n"; exit;}
